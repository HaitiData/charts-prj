        ...
        selection.each(function() {
            ...
            function toolTipHTML(data) {
                ...
                for (var key in data.data) {
                    ...
                    // if value is a number, format it as a float
                    var value = (!isNaN(data.data[key])) ? parseFloat(data.data[key]) : data.data[key];
            ...
            function calculatePoints(d) {
                ...
                var rev_pos = outerArc.centroid(d);
                pos[0] = radius * 0.95 * (midAngle(d) < Math.PI ? 1 : -1);
                // verify label conflict: if overlaps move up of 10
                if (pos[0] < 0 && pos[1] < 0) {
                    var i = 0;
                    if (occupedPoint.length >= 1){
                        for (i = 0; i < occupedPoint.length; i++){
                            if (pos[1] <= (occupedPoint[i] + 10) && (pos[1] >= (occupedPoint[i] - 10))){
                                    pos[1] = pos[1] - 20;
                                    rev_pos[1] = rev_pos[1] - 20;
                            }
                        }
                    }  occupedPoint.push(pos[1]);
                }
                return [arc.centroid(d), rev_pos, pos]
            }
            ...
            function labelTransform(d) {
                ...
                if (pos[0] < 0 && pos[1] < 0) {
                    var i = 0;
                    if (occupedLabel.length >= 1){
                        for (i = 0; i < occupedLabel.length; i++){
                            if (pos[1] <= (occupedLabel[i] + 10) && (pos[1] >= (occupedLabel[i] - 10))){
                                    pos[1] = pos[1] - 20;
                            }
                        }
                    }  occupedLabel.push(pos[1]);
                }
